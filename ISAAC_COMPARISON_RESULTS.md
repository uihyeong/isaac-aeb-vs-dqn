...
## 1.6. Ablation — pure-pursuit 대조군 (RL의 순수 기여 분리)

DQN 주행 = pursuit + RL회피. 회피를 끈 **pursuit-only**(`PURSUIT_ONLY=1`)와 비교 (각 방식 20런). 그래프: `figures/ablation_pursuit_vs_dqn_vs_aeb.png`.

| 방식 | 동행 충돌률 | 교차(가혹) 충돌률 | 동행 E-stop(평균) |
|:---:|:---:|:---:|:---:|
| Pursuit only (회피 X) | **35%** (7/20) | 15% (3/20) | 0 |
| DQN (RL) | **20%** (4/20) | 15% (3/20) | 0 |
| AEB (규칙) | 10% (2/20) | 20% (4/20) | 4.1 |

- **동행형: pursuit 35% → DQN 20%.** RL 회피가 충돌을 약 43% 감소(E-stop 0 유지) → **RL이 실제로 기여함 입증**.
- **교차형: pursuit ≈ DQN(15%).** 학습 분포 밖이라 RL 이점 없음.
- **트레이드오프:** AEB=최저충돌(10%)·고E-stop(jerky) / Pursuit=매끄럽지만 위험(35%) / **DQN=균형(매끄러움+충돌 절반)**.
